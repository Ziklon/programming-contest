...
public class ArrayUtils {
    ...
    public static int[] radixSort(int[] array, int n) {
        ...
        {
            int[] b = new int[65537];
            for (int i = 0; i < n; i++) b[1 + (array[i] & 0xffff)]++;
            for (int i = 1; i <= 65536; i++) b[i] += b[i - 1];
            for (int i = 0; i < n; i++) to[b[array[i] & 0xffff]++] = array[i];
            int[] d = array;
            array = to;
            to = d;
        }
        {
            int[] b = new int[65537];
            for (int i = 0; i < n; i++) b[1 + (array[i] >>> 16)]++;
            for (int i = 1; i <= 65536; i++) b[i] += b[i - 1];
            for (int i = 0; i < n; i++) to[b[array[i] >>> 16]++] = array[i];
            int[] d = array;
            array = to;
            to = d;
        }
    ...
    public static long[][] partialSums(int[][] array) {
        ...
        for (int i = 1; i <= height; i++) {
            for (int j = 1; j <= width; j++) {
                result[i][j] =
                        result[i][j - 1] + result[i - 1][j] - result[i - 1][j - 1] + array[i - 1][j - 1];
            }
        }
        return result;
    }
